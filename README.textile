h1. Robotis CM-900 (originally for Arduino) library for Pololu LSM303 boards
...
This is a library for the "Robotis CM-900 series":http://www.robotsource.org/bs/bd.php?bt=forum_CM9DeveloperWorld that interfaces with LSM303D, LSM303DLHC, LSM303DLM, and LSM303DLH 3D compass and accelerometer ICs on Pololu boards. It makes it simple to read the raw accelerometer and magnetometer data from these boards:
...
Make the following connections with wires between the CM-900 and the LSM303 board:

h4. CM-900

pre. CM-900      LSM303 board
-------------------------
              3.3V  ->  VIN
               GND  ->  GND
    Digital Pin 15  ->  SCL
...
- @void writeReg(byte reg, byte value)@ := Writes an accelerometer or magnetometer register with the given value. Register address constants are defined in LSM303.h.
- @void writeReg(byte reg, byte value)@ := Reads an accelerometer or magnetometer register and returns the value read.[2]
...
- @byte readMagReg(int reg)@ := Reads a magnetometer register and returns the value read. If the magnetometer data registers are read using register address constants without a specific device prefix (e.g. @OUT_Y_H_M@), this function will automatically use the correct register addresses depending on the device type.
